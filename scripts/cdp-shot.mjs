...
const PORT = Number(process.env.FRAME_CDP_PORT || 9222)
// Native dialogs or a disconnected WebView must not leave acceptance checks hanging.
const watchdog = setTimeout(() => {
  console.error('WebView check timed out after 30 seconds; inspect device/dialog state before retrying')
  process.exit(1)
}, 30000)
watchdog.unref()
...
if (cmd === 'eval-file') {
  ...
  console.log(JSON.stringify(await evalJs(readFileSync(rest[0], 'utf8'))))
  process.exit(0)
}
if (cmd === 'eval') {
  ...
  const ok = await evalJs(`(() => {
    ...
    const leaves = els.reverse().filter(e => e.childElementCount === 0)
    const el = leaves.find(e => e.textContent.trim() === ${text}) || leaves.find(e => e.textContent.trim().includes(${text}))
    if (!el) return false

...
export function templateThumbSvg(config: Partial<FrameConfig>, opts: ThumbOptions = {}): string {
  ...
  if (c.infoLayout === 'duo' || c.infoLayout === 'inline') {
    ...
    if (c.showPalette) {
      const colors = c.paletteColors ?? []
      const width = MAG_SWATCH_W * MAG_SWATCH_COUNT / colors.length
      for (let i = 0; i < colors.length; i++) {
        info += `<rect x="${r2(layout.palette.x + i * width)}" y="${r2(layout.palette.y)}" width="${r2(width)}" height="${r2(MAG_SWATCH_H)}" fill="${colors[i]}"/>`
      }
    }

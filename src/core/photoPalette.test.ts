...
import { extractPalette, FALLBACK_PALETTE, paletteFor, detectPalette } from './photoPalette'
...
describe('FALLBACK_PALETTE 与 paletteFor 缓存', () => {
  it('未读取到照片不显示固定假色', () => {
    expect(FALLBACK_PALETTE).toEqual([])
  })
...
it('支持不同色卡数量', () => {
  expect(extractPalette({} as HTMLCanvasElement,16,16,3)).toHaveLength(3)
  expect(extractPalette({} as HTMLCanvasElement,16,16,8)).toHaveLength(8)
})
it('读取失败拒绝并且不缓存假色', async () => {
  const OriginalImage = globalThis.Image
  globalThis.Image = class { onerror?: () => void; set src(_v:string) { queueMicrotask(() => this.onerror?.()) } } as unknown as typeof Image
  try {
    await expect(detectPalette('invalid:test')).rejects.toThrow()
    expect(paletteFor('invalid:test')).toEqual([])

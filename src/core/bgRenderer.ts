...
import { createRenderSurface, renderSurfaceFactory } from './renderSurface'

function imgWidth(img: ImgSource): number {
  if (img instanceof HTMLImageElement) return img.naturalWidth
  if (typeof SVGImageElement !== 'undefined' && img instanceof SVGImageElement) return img.width.baseVal.value
...
function imgHeight(img: ImgSource): number {
  ...
  if (typeof SVGImageElement !== 'undefined' && img instanceof SVGImageElement) return img.height.baseVal.value
...
export function drawBlurredBackground(
  ...
  const factory = renderSurfaceFactory(ctx)
  // Recording surfaces belong to a single plan and must never enter shared caches.
  ...
  const blurSrc = factory ? { src: img, w: iw0, h: ih0 } : getBlurSource(img, w, h, blurPx)
  const iw = blurSrc.w
  const ih = blurSrc.h
  ...
  // 在离屏画布中渲染模糊结果，供缓存复用
  const off = factory ? createRenderSurface(w, h, factory) : createOffscreen(w, h)
  ...
  if (!factory && w * h <= BLUR_CACHE_MAX_PX) {
 ...
 * 绘制水印（文本/图片，单一或平铺）。
 */
let watermarkTintCache: { source: CanvasImageSource; color: string; surface: HTMLCanvasElement } | null = null
export function drawWatermark(
  ...
  const sizePx = (Math.max(0.1, opts.size) / 100) * w
  let source = opts.image
  const surfaceFactory = renderSurfaceFactory(ctx)
  if (source && opts.tint && /^#[0-9a-f]{6}$/i.test(opts.tint)) {
    const iw = imgWidth(source), ih = imgHeight(source)
    if (!surfaceFactory && watermarkTintCache?.source === source && watermarkTintCache.color === opts.tint) {
      source = watermarkTintCache.surface
    } else if (iw && ih) {
      const surface = createRenderSurface(iw, ih, surfaceFactory)
      const tintCtx = surface.getContext('2d')!
      tintCtx.drawImage(source, 0, 0)
      tintCtx.globalCompositeOperation = 'source-in'
      tintCtx.fillStyle = opts.tint
      tintCtx.fillRect(0, 0, iw, ih)
      if (!surfaceFactory) watermarkTintCache = { source, color: opts.tint, surface }
      source = surface
    }
  } else watermarkTintCache = null
  ctx.save()
  ctx.globalAlpha = opts.opacity

  const drawOne = (cx: number, cy: number) => {
    if (source) {
      const iw = imgWidth(source)
      const ih = imgHeight(source)
      if (iw && ih) {
        const r = sizePx / Math.max(iw, ih)
        ctx.drawImage(source, cx - (iw * r) / 2, cy - (ih * r) / 2, iw * r, ih * r)
      }
    } else if (opts.text) {
  ...
  if (opts.tile) {
    ...
    let cx = w / 2
    let cy = h - opts.bottom
    if (opts.align === 'left') cx = opts.bottom
    if (opts.align === 'right') cx = w - opts.bottom
    if (opts.position && source) {
      const iw = imgWidth(source), ih = imgHeight(source)
      const ratio = sizePx / Math.max(1, iw, ih)
      const angle = opts.angle * Math.PI / 180
      const halfW = Math.min(w / 2, (Math.abs(iw * ratio * Math.cos(angle)) + Math.abs(ih * ratio * Math.sin(angle))) / 2)
      const halfH = Math.min(h / 2, (Math.abs(iw * ratio * Math.sin(angle)) + Math.abs(ih * ratio * Math.cos(angle))) / 2)
      cx = halfW + (w - 2 * halfW) * Math.max(0, Math.min(100, opts.position.x)) / 100
      cy = halfH + (h - 2 * halfH) * Math.max(0, Math.min(100, opts.position.y)) / 100
      ctx.translate(cx, cy)
      ctx.rotate(angle)
      drawOne(0, 0)
    } else drawOne(cx, cy)

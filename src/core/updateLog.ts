...
export const UPDATE_LOG: UpdateEntry[] = [
  {
    version: '0.3.3', date: '2026-09-16', importance: 'patch',
    groups: {
      fixed: ['修复 Android APK 启动时错误请求 localhost:5180：改为使用随包内置的前端资源', '移动端发布版本号与 Android versionCode 同步，避免安装旧测试包'],
      known: ['当前 APK 已完成本地构建与静态检查；尚未连接 vivo X100s 做真机验收'],
    },
  },
  {
    version: '0.3.1', date: '2026-09-15', importance: 'normal',
    groups: {
      added: ['杂志色卡从当前原图真实提取；INFO 中可重新提取、选择 2–8 色及手动调整，取消固定假色回退', '图库及编辑页“我的素材”右键创建虚拟副本，共享文件且独立保存编辑', 'Tab 三步切换面板：隐藏左右 → 显示左右并隐藏底栏 → 全部显示；F 切换全屏', '新增离线字体 Bebas Neue、Marcellus、Rajdhani'],
      improved: ['创意水印面板移到右侧最上方', '双击以实时鼠标位置放大；再次双击立即复位，偏移状态先归位'],
      fixed: ['模板示意图固定蓝灰色与真实照片不符', '面板尺寸变化后缩放锚点偏移，以及轻微鼠标移动干扰连续双击'],
      known: ['副本不是磁盘备份，原文件移动或删除会影响所有引用；网页临时导入的副本与原图一样不支持跨启动恢复', '新增三款字体主要用于拉丁字符，中文回退到系统中文字体；取色分析原图，不含裁剪和装饰'],
    },
  },
  {
    version: '0.3.0', date: '2026-09-15', importance: 'major',
    groups: {
      added: ['按照片独立记忆编辑；稳定路径标识，退出前等待保存，恢复上次撤销位置', '同步与复制支持按照片、背景、边框、创意水印、INFO、效果模块选择；同步和模板批量应用内置选片窗口', 'INFO 从 JPG、TIFF 和 TIFF/EXIF 型 RAW 原片读取拍摄信息，不替换当前图片', '五款离线开源字体：Cormorant Garamond、Cinzel、Oswald、马善政、站酷小薇', '补充保存、全选、取消全选、模块复制粘贴、适配、帮助快捷键；左上角版本号和更新日志入口'],
      fixed: ['重启后因照片 ID 改变而找不到编辑历史', '新图继承其他照片装饰，以及快速切图时参数串图的风险', '切换不同创意样式后品牌文字变回占位文案', '对齐不能重复点击及贴边问题：改为按钮加可调留白，默认 5%', '图库普通点击只加载却不进入编辑'],
      known: ['旧版历史仅在记录中可确认原图路径时自动恢复；无法确定归属的记录不强行关联', 'RAW 支持取决于文件内 EXIF 结构；不支持的格式请使用保留 EXIF 的相机 JPG/TIFF', '图片移动或改名后视为新路径；网页端临时导入图片不支持重启恢复图库'],
    },
  },
  {
    version: '0.2.8',

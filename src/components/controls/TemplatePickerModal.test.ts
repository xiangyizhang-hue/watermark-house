...
const mock = vi.hoisted(() => {
  ...
  return { mockTemplates, removeMock, applyMock, applyBatchMock, renderThumb, setPanelMock, mockItems, saveMock: vi.fn() }
})

// ---- composables mock（带数据，可被 beforeEach 重置，且测试间互不污染） ----
vi.mock('../../composables/useTemplates', () => ({
  useTemplates: () => ({ templates: mock.mockTemplates, remove: mock.removeMock, saveCurrent: mock.saveMock }),
...
describe('TemplatePickerModal 骨架', () => {
  it('offers mobile save in the combined library and reports a failed commit', async () => {
    const w = mountModal()
    await w.setProps({ allowSave: true })
    await w.find('.tp-save-inp').setValue('手机模板')
    mock.saveMock.mockImplementationOnce(() => { throw new Error('空间不足') })
    await w.find('.tp-save-btn').trigger('click')
    expect(w.find('[role="alert"]').text()).toContain('保存失败')
    expect(w.find('.tp-save-inp').element).toHaveProperty('value', '手机模板')
    await w.find('.tp-save-btn').trigger('click')
    expect(w.find('[role="status"]').text()).toContain('已保存')
    w.unmount()
  })
...
describe('批量与删除', () => {
  it('右栏批量按钮按选中照片数显示目标（选中1张）', async () => {
    const w = mountModal(true)
    expect(w.find('.batch-main').text()).toContain('选择照片')
    w.unmount()
  })
  ...
  it('点击批量按钮：applyTemplateToPhotos 收到选中照片 ids 与模板名', async () => {
    ...
    await flushPromises()
    expect(mock.applyBatchMock).not.toHaveBeenCalled()
    await w.findAll('dialog button').find(b => b.text() === '应用到 1 张')!.trigger('click')
    await flushPromises()
    expect(mock.applyBatchMock).toHaveBeenCalledWith(['p1'], expect.anything(), '应用模板「白框参数卡」')
    w.unmount()
  })

  it('删除模板必须先确认，点击删除按钮不应用模板', async () => {
    mock.removeMock.mockClear()
    const w = mountModal(true)
    const customCard = w.findAll('.tp-card')[2]
    await customCard.find('.tp-delete-action').trigger('click')
    expect(mock.removeMock).not.toHaveBeenCalled()
    expect(mock.applyMock).not.toHaveBeenCalled()
    const confirm = w.findAll('button').find(b => b.classes().includes('confirm') && b.text() === '删除')!

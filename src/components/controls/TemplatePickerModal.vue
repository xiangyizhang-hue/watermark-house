...
import { exportMobileTemplate, importMobileTemplate } from '../../platform/mobileBackup'
...
const props = withDefaults(
  ...
  { category: 'frame', customOnly: false, allowSave: false, title: '' },
)
...
function confirmDelete() {
  if (pendingDelete.value) removeCustom({ id: pendingDelete.value })
  pendingDelete.value = null
}
async function exchangeTemplate(exportId?: string) {
  if (fileBusy.value) return
  fileBusy.value = true; saveError.value = ''; savedTip.value = ''
  try {
    const ok = exportId ? await exportMobileTemplate(exportId) : await importMobileTemplate()
    savedTip.value = ok ? (exportId ? '模板文件已保存（含引用的自定义 Logo）' : '模板已导入到「我的模板」') : '已取消文件操作'
  } catch (e) { saveError.value = `文件操作失败：${String(e instanceof Error ? e.message : e)}` }
...
function onSaveCurrent() {
  ...
  try {
    ...
    saveError.value = `保存失败：${error instanceof Error ? error.message : String(error)}。请释放存储空间后重试。`
    return
  }
  saveName.value = ''
...
watch(
  () => [props.modelValue, list.value.map((t) => t.id).join(','), state.photoSrc, previewInfo.value] as const,
  () => {
    if (!props.modelValue) { ++thumbSeq; return }
    ...
    for (const t of list.value) {
      ...
      void (async () => {
        try {
          ...
          const url = await renderTemplateThumbDataUrl({ ...t.config, photoSrc: src }, ds, 480, info)
...
watch(
  () => [props.modelValue, selected.value?.id, previewInfo.value, state.photoSrc] as const,
  () => {
    if (!props.modelValue) { previewId.value = null; return }
    const t = selected.value
    if (!t) return
    const originalSrc = state.photoSrc
    ...
    void (async () => {
      try {
        const photoSrc = await photoDrawableSrc(originalSrc || null)
        const url = await renderTemplateThumbDataUrl({ ...t.config, photoSrc: originalSrc }, photoSrc, 960, previewInfo.value)
...
const batchDialog = ref<HTMLDialogElement>()
const batchTemplate = ref<{ id: string; name: string }>()
const batchBusy = ref(false)
const batchError = ref('')
function askBatch(t: { id: string; name: string }) { batchTemplate.value = t; batchError.value = ''; batchDialog.value?.showModal() }
async function confirmBatch() {
  if (!batchTemplate.value || !batchTargets.value.length || batchBusy.value) return
  batchBusy.value = true
  try { await applyBatch(batchTemplate.value); batchDialog.value?.close() } catch(e) { batchError.value = String(e) } finally { batchBusy.value = false }
}
...
function removeCustom(t: { id: string }) {
  try { templates.remove(t.id) } catch {
    saveError.value = '删除未能保存，模板已保留，请稍后重试。'
    return
  }
...
function doRename(newName: string) {
  try { if (renameId.value) templates.rename(renameId.value, newName) } catch {
    saveError.value = '重命名未能保存，原名称已保留，请稍后重试。'
    return
  }
...
<template>
      ...
      <div class="tp-modal">
        ...
        <div class="tp-body">
          ...
          <div class="tp-col tp-grid-col">
            <div v-if="isAndroid && isTauri" class="tp-file-actions">
              <button :disabled="fileBusy" @click="exchangeTemplate()">{{ fileBusy ? '处理中…' : '导入模板文件' }}</button>
              <button :disabled="fileBusy || !selected" @click="selected && exchangeTemplate(selected.id)">导出所选模板</button>
            </div>
            <!-- 我的模板模式：顶部内嵌「保存当前配置」表单 -->
            <div v-if="customOnly || allowSave" class="tp-save">
              <input
                v-model="saveName"
                class="tp-save-inp"
            ...
            <p v-if="savedTip" class="tp-save-tip" role="status">{{ savedTip }}</p>
            <p v-if="saveError" class="tp-save-tip" role="alert">{{ saveError }}</p>
              ...
              <div class="tp-grid">
                ...
                >
                  ...
                  <div class="tp-card-meta">
                    ...
                    <span class="tp-card-batch" title="选择照片并批量应用" @click.stop="askBatch(t)">⇉</span>
                  </div>
                </div>
              </div>
                ...
                >
                  ...
                  <div class="tp-card-meta">
                    ...
                    <div class="tp-card-actions" @click.stop>
                      <button class="tp-rename-action" aria-label="重命名模板" @click="askRename(t)">改名</button>
                      <button class="tp-delete-action" aria-label="删除模板" @click="pendingDelete = t.id">删除</button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="tp-col tp-preview-col">
            <template v-if="selected">
              ...
              <button class="btn batch-main" :disabled="library.items.length === 0" @click="askBatch(selected)">
                ⇉ 选择照片并批量应用
              </button>
            </template>
  ...
  <GlassModal :model-value="!!pendingDelete" title="删除模板" message="只删除这套模板，不改变照片当前的编辑效果。" confirm-text="删除" cancel-text="取消" :z-index="1200" @update:model-value="!$event && (pendingDelete = null)" @confirm="confirmDelete" />
  <dialog ref="batchDialog" class="workflow-dialog" aria-label="批量模板选择照片" @cancel="batchBusy && $event.preventDefault()">
    <h2>批量应用：{{ batchTemplate?.name }}</h2><p>只应用到下方勾选的照片。未勾选时不会自动应用到全部。</p>
    <PhotoSelection /><p role="alert">{{ batchError }}</p>
    <div class="workflow-actions"><button :disabled="batchBusy" @click="batchDialog?.close()">取消</button><button :disabled="batchBusy || !batchTargets.length" @click="confirmBatch">{{ batchBusy ? '正在保存…' : `应用到 ${batchTargets.length} 张` }}</button></div>
  </dialog>
...
<style scoped>
.tp-file-actions, .tp-card-actions { display:flex; flex-wrap:wrap; gap:8px; margin-block:8px; }
.tp-file-actions button, .tp-card-actions button { min-height:48px; padding:8px 12px; border:1px solid var(--border); border-radius:10px; color:var(--text); background:var(--hover); font-size:14px; }
.tp-file-actions button { flex:1; }
.tp-card-actions button { flex:1; }
.tp-file-actions button:disabled { opacity:.5; }
.tp-file-actions button:active, .tp-card-actions button:active { background:var(--accent); }
.tp-mask {
...
.tp-card-meta { display: flex; flex-wrap:wrap; align-items: baseline; gap: 6px; padding: 5px 8px 6px; }
.tp-card-actions { flex-basis:100%; }

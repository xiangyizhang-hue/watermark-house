...
function fit() {
  ...
  lastPhotoW = photoW
  lastPhotoH = photoH
  void nextTick(measureWrapOrigin)
}
...
function scheduleFit() {
  if (!fitRaf) fitRaf = requestAnimationFrame(() => { fitRaf = 0; fit() })
}
...
// Defer layout writes out of ResizeObserver delivery to prevent observer loops
// when the Android side panel changes the available stage width.
onMounted(() => {
  fit()
  if (stage.value && 'ResizeObserver' in window) { stageRO = new ResizeObserver(scheduleFit); stageRO.observe(stage.value) }
...
function onDblClick(e: MouseEvent) {
  cancelAnimationFrame(wheelRaf); wheelRaf = 0; pendingFactor = 1
  pointerStart = null; dragging.value = false
  if (Math.abs(viewer.zoom.value - 1) > 0.01 || Math.abs(viewer.panX.value) > 1 || Math.abs(viewer.panY.value) > 1) {
    lastFrameW = 0
    fit()
    viewer.resetView()
    return
  ...
  // 当前可见矩形包含面板伸缩、滚动和最新 fit，避免使用过期布局坐标。
  const rect = wrap.getBoundingClientRect()
  viewer.setZoomAt(factor, e.clientX - rect.left, e.clientY - rect.top)
}
...
function onPointerMove(e: PointerEvent) {
  ...
  if (!dragging.value) {
    if (Math.hypot(e.clientX-pointerStart.x,e.clientY-pointerStart.y) < 4) return
    dragging.value = true
...
<template>
  <section class="workspace">
    <div
      ...
      @lostpointercapture="pointerStart = null; dragging = false"

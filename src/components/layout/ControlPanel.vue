...
import CreativeWatermarkPanel from '../controls/CreativeWatermarkPanel.vue'

const app = useAppState()
const library = useLibrary()
const { reset, patch } = useFrameConfig()
const props = defineProps<{
  /** 移动端 bottom sheet 当前只呈现一个功能分组；桌面端不传则保持完整右栏。 */
  mobileSection?: 'creative' | 'photo' | 'background' | 'border' | 'info'
}>()
...
<template>
  <aside class="right-panels" :style="{ width: app.rightWidthPx.value }">
    ...
    <!-- 顶部：单行线性图标工具栏（移动端由 bottom sheet 自己提供） -->
    <div v-if="!props.mobileSection" class="panel-toolbar">
    ...
    <!-- 移动端：bottom sheet 内直接显示单个面板，避免在小屏嵌套多层折叠。 -->
    <div v-if="props.mobileSection" class="mobile-panel-list">
      <CreativeWatermarkPanel v-if="props.mobileSection === 'creative'" />
      <ImageLayout v-else-if="props.mobileSection === 'photo'" />
      <BackgroundMode v-else-if="props.mobileSection === 'background'" />
      <BorderSettings v-else-if="props.mobileSection === 'border'" />
      <InfoLayerPanel v-else />
    </div>

    <!-- 桌面面板组：纵向单列，细分割线分隔（由内向外：照片 → 背景 → 边框 → INFO） -->
    <div v-else class="panel-list">
      <CreativeWatermarkPanel />
      <!-- 1. 照片 -->
      <CollapsiblePanel
        title="照片"
        :open="isOpen('photo')"

...
// 视觉沿用 水印小屋 的方正极简语言（细边框、直角、小字号），开关控件用圆角胶囊以提升可辨识度。
// 桌面端由原生菜单「文件 → 首选项…」/ Ctrl+, 触发；网页端通过顶栏 ⚙ 打开，两侧均可访问。
import { ref, computed, onMounted, onBeforeUnmount } from 'vue'
import { isDesktopTauri, isAndroid, isTauri } from '../../platform/env'
import MobileBackupPanel from '../mobile/MobileBackupPanel.vue'
import {
  listGpus,
  getGpuSelection,
  setGpuSelection,
...
async function runClear(kind: string) {
  ...
  if (kind === 'library') {
    ...
    try { templates.clearCustom() } catch {
      window.alert('模板清理未能保存，原模板已保留，请稍后重试。')
    }
  }
}
...
const GREEN_RELEASES_URL = 'https://github.com/xiangyizhang-hue/watermark-house/releases/latest'
    ...
    <div class="pref-box">
      ...
      <div class="pref-body">
        ...
        <section class="pf-sec">
          ...
          <div class="pf-row" v-if="isDesktopTauri">
            <div class="pf-text">
          ...
          <div class="pf-row" v-if="isDesktopTauri">
            <div class="pf-text">
              ...
              <span class="pf-desc">实际显卡由系统/驱动分配，可在此手动为 水印小屋 指定高性能。</span>
            </div>
        ...
        <section class="pf-sec">
          ...
          <MobileBackupPanel v-if="isAndroid && isTauri" />
          <div class="pf-row">
            <div class="pf-text">
        ...
        <section class="pf-sec">
          ...
          <div class="pf-row" v-if="isDesktopTauri">
            <div class="pf-text">
              ...
              <span class="pf-desc">退出并重新启动 水印小屋，用于让性能、数据类设置彻底生效。</span>
            </div>
          ...
          <div class="pf-about">
            <span class="pf-about-logo">◎ 水印小屋</span>
            <span class="pf-about-desc">作者：风喃（fengnan）</span>

...
import { drawCreativeWatermark } from '../../core/creativeWatermark'
import { loadWatermarkFonts } from '../../core/watermarkFonts'
...
async function ensureWmImage() {
  if (state.watermarkImage && (!wmImg || (wmImg as HTMLImageElement).src !== state.watermarkImage)) {
    ...
    if (im.complete) wmImg = im
      ...
      if (state.watermarkImage === im.src) wmImg = im.naturalWidth ? im : null
    }
...
function render() {
  ...
  if (state.showWatermark) {
    drawWatermark(ctx, w, h, {
      ...
      tint: state.watermarkTint,
      position: state.watermarkPosition,
    })
  }
  drawCreativeWatermark(ctx, w, h, state.creativeWatermark)
}
...
function scheduleRender() {
  ...
  renderRaf = requestAnimationFrame(() => {
    ...
    void (async () => {
      await ensureWmImage()
      await loadWatermarkFonts(state.creativeWatermark).catch(() => {})
      nextTick(render)
    })()
  })
}

...
export interface CatalogData {
  ...
  copies?: { id: string; path: string; name: string }[]
  ...
  try {
    if (raw) {
      ...
      if (obj && typeof obj === 'object') {
        ...
        const copies = Array.isArray(obj.copies) ? obj.copies.filter(c => c && typeof c.id === 'string' && c.id.startsWith('copy:') && typeof c.path === 'string' && typeof c.name === 'string') : []
        return { folder, paths, activePath, meta, copies, activeId: typeof obj.activeId === 'string' ? obj.activeId : null }
      }
    }
  } catch {
    /* 损坏数据按空目录处理 */
...
function isEmpty(d: CatalogData): boolean {
  return !d.paths.length && !d.copies?.length && !d.folder && !d.activePath
}
...
function persist(): void {
  ...
  writeTimer = setTimeout(() => {
    writeTimer = null
    void flushPersist().catch(e => console.warn('[catalog] 保存失败', e))
  }, WRITE_DEBOUNCE_MS)
}
...
export function flushPersist(): Promise<void> {
  ...
  writeInFlight = Promise.resolve().then(async () => {
    try {
      while (dirty && cache) {
        dirty = false
        const json = JSON.stringify(cache)
        if (!isDesktopTauri) localStorage.setItem(CATALOG_KEY, json)
        else {
...
export async function ensureCatalogLoaded(): Promise<void> {
  if (!isDesktopTauri || loaded) return
...
export function catalogAddCopy(copy: { id: string; path: string; name: string }): void {
  const d = read()
  d.copies ??= []
  if (!d.copies.some(c => c.id === copy.id)) { d.copies.push({ ...copy }); persist() }
}
export function catalogRemoveCopy(id: string): void {
  const d = read()
  d.copies = (d.copies ?? []).filter(c => c.id !== id)
  persist()
}
...
export function catalogPruneMeta(): void {
  const d = read()
  if (!d.meta) return
  const keep = new Set([...d.paths, ...(d.copies ?? []).map(c => c.path)])
  let changed = false
...
export function catalogSetActive(path: string | null, id: string | null = null): void {
  const d = read()
  if ((d.activePath ?? null) === path && (d.activeId ?? null) === id) return
  cache = { ...d, activePath: path, activeId: id }
  persist()
}

...
async function ensureAssetApi(): Promise<void> {
  if (!isDesktopTauri || convertFileSrcFn) return
...
export function assetUrl(path: string): string {
  return isDesktopTauri && convertFileSrcFn ? convertFileSrcFn(path) : path
}
...
export async function toDrawableUrl(url: string): Promise<string> {
  if (!isDesktopTauri || !isAssetProtocolUrl(url)) return url
...
export async function restoreLibrary(): Promise<void> {
  ...
  if (!cat.paths.length && !cat.copies?.length) {
  ...
  for (const p of new Set([...cat.paths, ...(cat.copies ?? []).map(c => c.path)])) {
    const m = p.match(/^(.*)[/\\][^/\\]+$/)
    let dir = m ? m[1] : ''
  ...
  for (const [dir, paths] of byDir) {
    ...
    for (const p of paths) {
      ...
      if (cat.paths.includes(p)) entries.push({ path: p, name: p.split(/[\\/]/).pop() || p, mtime: stat?.mt, size: stat?.sz })
      for (const copy of cat.copies ?? []) {
        if (copy.path === p) entries.push({ ...copy, mtime: stat?.mt, size: stat?.sz })
      }
    }
  }

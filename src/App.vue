...
import { isDesktopTauri, isMobileViewport } from './platform/env'
...
let loadedPhotoId: string | null = null
...
async function loadActive() {
  ...
  if (loadingUrl === active.id) {
    return
  }
  const seq = ++switchSeq
  loadingUrl = active.id
  ...
  try {
    ...
    if (loadingUrl === active.id) loadingUrl = null
    suspendCommit(false)
...
function hasOpenModal(): boolean {
  const sel =
    'dialog[open], .modal-mask, .pref-mask, .upd-mask, .tp-mask, .editor-mask, .preview-mask, .guide-mask, .rt-err-mask'
...
function onKey(e: KeyboardEvent) {
  ...
  if (!mod && !e.altKey && !e.repeat && key === 'f') { e.preventDefault(); void toggleFullscreen().catch(error => useParamClipboard().showResult(`全屏切换失败：${String(error)}`)); return }
  if (!mod && !e.altKey && !e.shiftKey && !e.repeat && e.key === 'Tab' && app.activeModule.value === 'develop') { e.preventDefault(); app.cyclePanels(); return }
  if (mod && key === 'a') { e.preventDefault(); if(e.shiftKey) library.selectNone(); else library.selectAll(); return }
  if (mod && e.shiftKey && key === 'c') { e.preventDefault(); useParamClipboard().requestCopy(); return }
  if (mod && e.shiftKey && key === 'v') { e.preventDefault(); void useParamClipboard().pasteParams(); return }
  if (mod && key === 's') {
    e.preventDefault()
    void history.saveAllPending().then(() => useParamClipboard().showResult('当前照片的编辑已保存。')).catch(e => useParamClipboard().showResult(`保存失败：${String(e)}`))
    return
  }
  if (mod && key === '0') { e.preventDefault(); useViewer().resetView(); return }
  if (e.key === 'F1') { e.preventDefault(); window.dispatchEvent(new Event('framelab-help')); return }
...
// Teleported mobile dialogs (模板库 / 首选项 / 工作流弹窗) 不再位于
// `.mobile-shell` 子树内，给 body 加平台类让它们也能使用同一套手机布局。
if (typeof document !== 'undefined') document.body.classList.toggle('is-mobile-app', mobileMode)
...
<template>
  <div class="app-root" :class="{ 'is-mobile-app': mobileMode }">
    <MobileShell v-if="mobileMode" :photo-src="photoSrc" :bg-image="bgImage" />
    <template v-else>
    <TopBar />

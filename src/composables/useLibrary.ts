...
import { importPhoto, removePhotoHistory, duplicatePhotoHistory, useHistory } from './useHistory'
...
import {
  ...
  saveMobileAsset,
  updateMobileAsset,
  type MobileAssetRecord,
} from '../platform/mobileAssets'
import {
  catalogAdd,
  catalogAddCopy,
  catalogRemoveCopy,
...
export interface LibraryItem {
  virtualCopy?: boolean
  /** 移动端虚拟副本的原图 id；副本链始终指向原图，不复制 Blob。 */
...
function remove(id: string): void {
  ...
  if (removed.virtualCopy) {
    if (isMobile) {
      void deleteMobileCopy(id).catch((error) => {
        reportRuntimeError('副本记录移除失败', `${removed.name} 已从当前图库隐藏，但副本记录未能清理：${String(error)}`)
      })
    } else {
      catalogRemoveCopy(id)
    }
  }
  else {
    catalogRemove(removed.path)
    if (isMobile) {
      void deleteMobileAsset(id).catch((error) => {
        reportRuntimeError('原图移除失败', `照片已从当前图库隐藏，但原图记录未能清理：${String(error)}`)
      })
    }
  }
  items.splice(idx, 1)
  if (!items.some(i => i.url === removed.url)) releaseUrl(removed.url)
  if (!items.some(i => i.thumbUrl === removed.thumbUrl)) releaseUrl(removed.thumbUrl)
...
watch(activeId, () => {
  ...
  catalogSetActive(it?.path ?? null, it?.id ?? null)
  try {
...
export function useLibrary() {
  ...
  async function addFiles(files: File[]): Promise<void> {
    ...
    for (const file of files) {
      ...
      // Android 不把 Photo Picker 的临时 URI 当作长期引用：先将原图写入
      // IndexedDB，事务成功后才把它加入图库。这样导入成功的定义是真正可恢复，
      // 不是「当前页面暂时能看到」。
      if (isMobile) {
        const now = Date.now()
        const record: MobileAssetRecord = {
          id,
          name: file.name,
        ...
        try {
          await saveMobileAsset(record)
          originalPath = record.originalPath
          if (record.thumbPath) nativeThumb = createMobileAssetUrl(undefined, record.thumbPath)
        } catch (error) {
          rejected.push(`${file.name}（原图保存失败：${String(error)}）`)
          continue
        }
      }
      let url: string
      try {
        url = isMobile ? createMobileAssetUrl(file, originalPath) : URL.createObjectURL(file)
      } catch (error) {
        if (isMobile) {
          await deleteMobileAsset(id).catch(() => {})
        }
        rejected.push(`${file.name}（无法建立本地预览：${String(error)}）`)
        continue
      }
      ...
      if (!nativeThumb) void makeThumbUrl(url, width, height).then((t) => {
        if (!t) return
      ...
      if (isMobile) {
        try {
          await updateMobileAsset(id, { exif: item.exif, updatedAt: Date.now() })
        } catch (error) {
          reportRuntimeError('照片信息保存失败', `${file.name} 的 EXIF 已读取，但未能写入本地记录：${String(error)}`)
        }
      }
      const exifPatch = item.exif ? buildExifPatch(item.exif) : null
      const { state, patch } = useFrameConfig()
      let snap = JSON.parse(JSON.stringify(defaultFrameConfig)) as (typeof state)
      if (exifPatch) snap = { ...snap, ...exifPatch } as typeof snap
      Object.assign(snap, { showLogo: false, showCameraModel: false, showExif: false, showLens: false, showDate: false })
  ...
  async function addLocalEntries(entries: LocalImageEntry[]): Promise<LibraryItem[]> {
    if (!isDesktopTauri || !entries.length) return []
    const {
      assetUrl,
      readLocalBytes,
      readLocalHead,
    ...
    const fresh = entries.filter((e) => e.path && !known.has(e.id ?? photoIdentity(e.path)))
    ...
    while (pending.length) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        const id = e.id ?? photoIdentity(e.path)
        if (firstNewId === null) firstNewId = id
    ...
    if (activeId.value === null) {
      ...
      const catActive = catalog?.activePath ?? null
      const hit = items.find(i => i.id === catalog.activeId) ?? (catActive ? items.find((i) => i.path === catActive) : undefined)
  ...
  function clearAll(): void {
    ...
    catalogClear()
    if (isMobile) {
      void clearMobileAssets().catch((error) => {
        reportRuntimeError('原图清理失败', `图库已经清空显示，但本地原图仓库未能完全清理：${String(error)}`)
      })
      void clearMobileCopies().catch((error) => {
        reportRuntimeError('副本清理失败', `图库已经清空显示，但虚拟副本记录未能完全清理：${String(error)}`)
      })
    }
  }
  ...
  async function createCopy(sourceId: string): Promise<void> {
    const source = items.find(i => i.id === sourceId)
    if (!source) return
    const id = `copy:${crypto.randomUUID()}`
    const extension = source.name.match(/\.[^.]+$/)?.[0] ?? ''
    const baseName = (extension ? source.name.slice(0,-extension.length) : source.name).replace(/ · 副本 \d+$/, '')
    let number = 1
    while (items.some(i => i.name === `${baseName} · 副本 ${number}${extension}`)) number++
    await duplicatePhotoHistory(sourceId, id)
    if (!items.includes(source)) { await removePhotoHistory(id); return }
    const sourceRootId = source.virtualSourceId ?? source.id
    if (isMobile) {
      try {
        const now = Date.now()
        await saveMobileCopy({
          id,
          sourceId: sourceRootId,
          name: `${baseName} · 副本 ${number}${extension}`,
          createdAt: now,
          updatedAt: now,
        })
      } catch (error) {
        await removePhotoHistory(id)
        throw new Error(`副本记录保存失败：${String(error)}`)
      }
    }
    const copy = reactive<LibraryItem>({ ...source, id, name: `${baseName} · 副本 ${number}${extension}`, selected: false, virtualCopy: true, virtualSourceId: sourceRootId, exif: source.exif ? JSON.parse(JSON.stringify(source.exif)) : null })
    items.splice(items.indexOf(source) + 1, 0, copy)
    if (!isMobile && copy.path) { catalogAddCopy({ id, path: copy.path, name: copy.name }); await flushPersist() }
  }
...
/**
 * Android 启动恢复：从应用私有原图仓库重建图库条目，再由历史链恢复每张照片的
 * 编辑参数。此函数必须在 restoreActive 前完成，否则首次启动时找不到目标照片。
 */
export async function restoreMobileAssets(): Promise<void> {
  if (!isMobile) return
  let records: MobileAssetRecord[]
  try {
    records = await listMobileAssets()
  } catch (error) {
    reportRuntimeError('图库恢复失败', `无法读取手机本地原图仓库：${String(error)}`)
    return
  }
  let copies: Awaited<ReturnType<typeof listMobileCopies>>
  try {
    copies = await listMobileCopies()
  } catch (error) {
    reportRuntimeError('副本恢复失败', `无法读取手机本地虚拟副本记录：${String(error)}`)
    copies = []
  }
  if (!records.length && !copies.length) return

  const library = useLibrary()
  const { state } = useFrameConfig()
  const restored = new Map<string, LibraryItem>()
  for (const record of records) {
    if (items.some((item) => item.id === record.id)) continue
    let url: string
    try {
      url = createMobileAssetUrl(record.blob, record.originalPath)
    } catch (error) {
      reportRuntimeError('照片恢复失败', `${record.name} 无法建立预览：${String(error)}`)
      continue
    }
    const file = mobileRecordFile(record)
    const exif = (record.exif ?? null) as ExifParseResult | null
    const item = reactive<LibraryItem>({
      id: record.id,
      name: record.name,
      url,
      path: record.originalPath,
      thumbUrl: record.thumbPath ? createMobileAssetUrl(undefined, record.thumbPath) : undefined,
      width: record.width,
    ...
    items.push(item)
    restored.set(record.id, item)
    if (!item.thumbUrl) void makeThumbUrl(url, record.width, record.height).then((thumb) => {
      if (!thumb) return
      if (!items.includes(item)) {
        releaseUrl(thumb)
        return
    ...
    // 存量记录可能来自较早版本，补解析一次 EXIF 并写回；失败不影响原图恢复。
    if (!item.exif && file) {
      item.exif = await library.parseExifQuiet(file)
      if (item.exif) {
        await updateMobileAsset(record.id, { exif: item.exif, updatedAt: Date.now() }).catch((error) => {
          reportRuntimeError('照片信息保存失败', `${record.name} 的 EXIF 无法写回本地记录：${String(error)}`)
        })
      }
    }
    const exifPatch = item.exif ? library.buildExifPatch(item.exif) : null
    let snapshot = JSON.parse(JSON.stringify(defaultFrameConfig)) as typeof state
    if (exifPatch) snapshot = { ...snapshot, ...exifPatch } as typeof snapshot
    // 新照片/没有历史的照片保持「无装饰」；EXIF 仅作为该照片的隐藏数据，
    // 由用户在 INFO 面板主动打开显示，避免重启后突然出现默认边框或信息层。
    ...
    await importPhoto(record.id, snapshot, '导入')
  }

  // 副本恢复放在原图之后：同一 Blob 只保留一份，副本只建立新的 URL 和独立历史链。
  const history = useHistory()
  for (const copyRecord of copies) {
    if (items.some((item) => item.id === copyRecord.id)) continue
    const source = restored.get(copyRecord.sourceId) ?? items.find((item) => item.id === copyRecord.sourceId)
    if (!source) {
      // 原图已被移除时清理孤儿副本元数据，避免每次启动重复报错。
      await deleteMobileCopy(copyRecord.id).catch(() => {})
      continue
    }
    let url: string
    try {
      url = createMobileAssetUrl(source.file ?? undefined, source.path)
    } catch (error) {
      reportRuntimeError('副本恢复失败', `${copyRecord.name} 无法建立预览：${String(error)}`)
      continue
    }
    const item = reactive<LibraryItem>({
      ...source,
      ...
      virtualCopy: true,
      virtualSourceId: copyRecord.sourceId,
      selected: false,
    })
    items.push(item)
    if (!item.thumbUrl) void makeThumbUrl(url, item.width, item.height).then((thumb) => {
      if (!thumb) return
      if (!items.includes(item)) {
        releaseUrl(thumb)
        return
    ...
    const sourceConfig = await history.getConfigForPhoto(copyRecord.sourceId)
    await importPhoto(copyRecord.id, sourceConfig ?? JSON.parse(JSON.stringify(defaultFrameConfig)), '创建副本')
  }
}

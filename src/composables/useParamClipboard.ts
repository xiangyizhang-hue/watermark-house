...
import { sanitizeTemplateConfig } from './useTemplates'
import { useFrameConfig } from './useFrameConfig'
import { applyModulesToPhotos } from './useHistory'
import { configModules, pickModules, mergeModuleConfig, type ConfigModule } from '../core/configModules'
...
const copyModules = ref<ConfigModule[]>(configModules.map(m => m.id))
...
function parseParams(text: string): Partial<FrameConfig> | null {
  try {
    ...
    if (c && typeof c === 'object' && !Array.isArray(c) && Object.keys(c).some(k => k in defaultFrameConfig)) {
      return sanitizeTemplateConfig(c)
    }
    return null
  } catch {
    return null
...
export function useParamClipboard() {
  ...
  async function copyParams(): Promise<boolean> {
    if (!copyModules.value.length) return false
    const text = JSON.stringify({ kind: 'framelab-params', version: 2, config: pickModules(state, copyModules.value) })
    try {
  ...
  async function pasteParams(): Promise<void> {
    ...
    if (!cfg) {
      const manual = window.prompt('未从剪贴板读到有效参数。可将参数 JSON 粘贴到输入框后确定（先在 水印小屋 点「复制参数」生成）：')
  ...
  return {
    copyOpen,
    copyModules,
    requestCopy: () => { copyOpen.value = true },
    syncTargets,
    pasteConfirmOpen,
    syncResultOpen,
    syncResultMsg,

...
import { mergeModuleConfig } from '../core/configModules'
import { isMobile } from '../platform/env'
import { markMobileDirty, markMobileSaveError, markMobileSaved, mobileEditRevision } from '../platform/mobileLifecycle'
import {
  loadPhotoNodes,
  loadHistoryCursor,
  saveHistoryCursor,
...
function persistNode(node: HistoryNodeRecord): void {
  const job = putHistoryNode(node).catch(e => { writeFailure = e; reportDbError(e) })
  writes.add(job)
  void job.finally(() => writes.delete(job))
}
export async function saveAllPending(): Promise<void> {
  ...
  try {
    ...
    if (isMobile) markMobileSaved(revision)
  } catch (error) {
    if (isMobile) markMobileSaveError()
    throw error
  }
}
function saveCursor(photoId: string): void {
  const node = chains[photoId]?.[cursors[photoId] ?? -1]
  if (node) {
    const job = saveHistoryCursor(photoId, node.id).catch(e => { writeFailure = e; reportDbError(e) })
    writes.add(job)
    void job.finally(() => writes.delete(job))
  }
}
...
export function ensureChain(photoId: string): Promise<void> {
  ...
  const p = (async () => {
    let recs = await loadPhotoNodes(photoId)
    if (!recs.length) recs = await recoverLegacyNodes(photoId)
    chains[photoId] = recs
    cursors[photoId] = recs.length > 0 ? recs.length - 1 : -1
    {
      const saved = await loadHistoryCursor(photoId)
      const index = recs.findIndex(n => n.id === saved)
      if (index >= 0) cursors[photoId] = index
    }
    loaded.add(photoId)
...
/**
 * 读取某张照片当前历史游标对应的配置，但不切换当前照片，也不改动全局画布。
 *
 * 批量导出在开始时用它冻结每张照片自己的编辑结果，避免导出循环中读取正在
 * 变化的全局 state，或把当前照片的水印误套到其它照片上。
 */
export async function getConfigForPhoto(photoId: string): Promise<FrameConfig | null> {
  await flushPending()
  await ensureChain(photoId)
  const node = chainOf(photoId)[cursors[photoId] ?? -1]
  return node ? (JSON.parse(JSON.stringify(node.state)) as FrameConfig) : null
}
...
function commitHistory(key = ''): void {
  ...
  const target = currentTarget()
  if (!target) return
  if (isMobile) markMobileDirty()
  pendingKey = { photoId: target.id, name: describe(key) }
  // Capture while the edited photo is still active, never after an async photo switch.
  snapshotNow(target.id, describe(key))
  ...
  if (!snapshotRaf) {
    snapshotRaf = requestAnimationFrame(() => {
      ...
      if (currentTarget()?.id === pendingKey.photoId) snapshotNow(pendingKey.photoId, pendingKey.name)
    })
  }
  if (debounceTimer) clearTimeout(debounceTimer)
  debounceTimer = setTimeout(() => { void saveAllPending().catch(reportDbError) }, HISTORY_DEBOUNCE_MS)
}
...
export async function flushPending(): Promise<void> {
  ...
  if (pendingKey && currentTarget()?.id === pendingKey.photoId && !restoring && !isCommitSuspended()) {
...
export async function duplicatePhotoHistory(sourceId: string, copyId: string): Promise<void> {
  await saveAllPending()
  await ensureChain(sourceId)
  const source = chainOf(sourceId)[cursors[sourceId] ?? -1]?.state
  if (!source) throw new Error('未找到原照片的编辑状态')
  await importPhoto(copyId, JSON.parse(JSON.stringify(source)), '创建副本')
  await saveAllPending()
}
...
export async function applyTemplateToPhotos(
  ...
  for (const id of photoIds) {
    ...
    if (config.brand?.startsWith('custom:')) next.brand = config.brand
    ...
    if (next.showLogo && !next.exifRaw && !next.brand.startsWith('custom:')) {
...
export async function applyModulesToPhotos(photoIds: string[], config: Partial<FrameConfig>, name = '同步选中模块'): Promise<void> {
  await flushPending()
  for (const id of new Set(photoIds)) {
    await ensureChain(id)
    const base = chainOf(id)[cursors[id] ?? -1]?.state
    if (!base) throw new Error('目标照片尚未完成导入，请稍后重试')
    const next = mergeModuleConfig(base, config)
    if ('padding' in config || 'borderRatio' in config) recalcCanvasHAfterTemplate(base, next)
    recordEdit(id, next, name)
  }
  const target = currentTarget()
  if (target && photoIds.includes(target.id)) loadCursorFor(target.id)
  await saveAllPending()
}

...
import { normalizeWatermark } from '../core/creativeWatermark'
...
function persist(next: FrameTemplate[]) {
  const custom = next.filter((t) => !t.builtin)
  localStorage.setItem(STORAGE_KEY, JSON.stringify(custom))
  // Publish only after durable browser storage accepted the complete snapshot.
...
function toTemplateConfig(cfg: FrameConfig): Partial<FrameConfig> {
  ...
  void lensText
  void brand
  // EXIF camera brands follow the target photo; a user-uploaded logo is part of
  // the template design and must survive save/export/import.
  return JSON.parse(JSON.stringify({ ...tpl, ...(brand.startsWith('custom:') ? { brand } : {}) }))
}
...
export function sanitizeTemplateConfig(raw: Partial<FrameConfig>): Partial<FrameConfig> {
  ...
  for (const [k, v] of Object.entries(raw)) {
    if (k === 'creativeWatermark') {
      const watermark = normalizeWatermark(v)
      if (watermark) out[k] = watermark
      continue
    }
      const base = def[k]
      if (k === 'paletteColors' && Array.isArray(v)) {
        out[k] = v.filter(c => typeof c === 'string' && /^#[0-9a-f]{6}$/i.test(c)).slice(0, 8)
        continue
      }
    if (base === undefined) continue // 未知键：丢弃
    if (k === 'exifRaw' && v !== null && typeof v === 'object' && !Array.isArray(v)) {
      const numericKeys = ['focalLength', 'focalLength35', 'fNumber', 'exposureTime', 'iso']
      const stringKeys = ['dateTimeOriginal', 'lensModel', 'lensMake', 'model', 'brandId']
      out[k] = Object.fromEntries(Object.entries(v).filter(([key, value]) =>
        numericKeys.includes(key) ? typeof value === 'number' && Number.isFinite(value) :
        stringKeys.includes(key) && typeof value === 'string'))
      continue
    }
    if (base === null && v !== null) {
      const numeric = /(?:X|Y|Top|Bottom|Size|Weight|Opacity)$/.test(k) || k === 'frameRatio'
      if (numeric ? typeof v === 'number' && Number.isFinite(v) : typeof v === 'string') out[k] = v
      continue
    }
    if (v === null) {
    ...
    if (typeof v === 'object') {
      if (!Array.isArray(v) && base !== null && typeof base === 'object') out[k] = JSON.parse(JSON.stringify(v))
      continue
    }
...
export function applyTemplateToState(rawConfig: Partial<FrameConfig>): string[] {
  ...
  const next: FrameConfig = {
    ...
    showBackground: config.showBackground ?? state.showBackground,
    showBorder: config.showBorder ?? state.showBorder,
    showInfo: config.showInfo ?? state.showInfo,
    ...
    brand: config.brand?.startsWith('custom:') ? config.brand : state.brand,
  ...
  if ((config.showLogo ?? state.showLogo) && !next.exifRaw && !next.brand.startsWith('custom:')) {
...
export function useTemplates() {
  function mergeRestored(restored: FrameTemplate[]): void {
    const existing = new Set(templates.map(t => t.id))
    persist([...restored.filter(t => !existing.has(t.id)).map(t => ({
      id: t.id, name: t.name, category: t.category,
      config: toTemplateConfig({ ...defaultFrameConfig, ...sanitizeTemplateConfig(t.config) }),
    })), ...templates])
  }
  ...
  function remove(id: string): void {
    ...
    if (idx >= 0 && !templates[idx].builtin) {
      persist(templates.filter((_, index) => index !== idx))
    }
  }
  ...
  function rename(id: string, name: string): boolean {
    ...
    persist(templates.map(item => item.id === id ? { ...item, name: trimmed } : item))
    return true
  }
  ...
  function clearCustom(): void {
    persist(templates.filter(item => item.builtin))
  }

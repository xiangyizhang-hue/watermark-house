...
const freshDefaults = () => ({ ...defaultFrameConfig, ...clone(defaultFrameConfig) })
const state = reactive<FrameConfig>(freshDefaults())
...
let batching = false
watch(state, () => { if (!batching && suspendDepth === 0) commitHook?.('编辑参数') }, { deep: true, flush: 'sync' })
function assign(...sources: Partial<FrameConfig>[]) {
  batching = true
  try { Object.assign(state, ...sources) } finally { batching = false }
}
...
export function useFrameConfig() {
  ...
  function loadConfig(partial: Partial<FrameConfig>): void {
    ...
    const norm =
      typeof lc === 'string' && !/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(lc.trim())
        ...
        : partial
    assign(freshDefaults(), clone(norm))
  ...
  /** 重置为默认 */
  function reset(): void {
    const { photoSrc, exifRaw, cameraModel, exifText, dateText, lensText, brand } = state
    assign(freshDefaults(), { photoSrc, exifRaw, cameraModel, exifText, dateText, lensText, brand })
    if (suspendDepth === 0) commitHook?.('reset')
  }

  /** Mobile reset is a single undoable edit, not a suppressed import operation. */
  function resetUndecorated(): void {
    const { photoSrc, exifRaw, cameraModel, exifText, dateText, lensText, brand } = state
    const defaults = freshDefaults()
    assign(defaults, { photoSrc, exifRaw, cameraModel, exifText, dateText, lensText, brand }, {
      showBackground: false, showBorder: false, showInfo: false,
      showLogo: false, showCameraModel: false, showExif: false,
      showLens: false, showDate: false, showWatermark: false,
      creativeWatermark: undefined,
      infoLayer: { ...defaults.infoLayer, enabled: false },
    })
    if (suspendDepth === 0) commitHook?.('reset')
  }

  return { state, loadConfig, patch, reset, resetUndecorated, suspendCommit }
}

...
#[cfg(desktop)]
const GITHUB_REPO_URL: &str = "https://github.com/xiangyizhang-hue/watermark-house";
/// 意见反馈：GitHub 新建 Issue（帮助菜单 → 意见反馈；邮箱直接显示在菜单中）
#[cfg(desktop)]
const GITHUB_ISSUES_URL: &str = "https://github.com/xiangyizhang-hue/watermark-house/issues/new";
...
#[tauri::command]
#[cfg(desktop)]
async fn pick_metadata_file(app: AppHandle) -> Result<Option<String>, String> {
    match app.dialog().file().add_filter("原始拍摄信息", &["jpg", "jpeg", "tif", "tiff", "nef", "nrw", "arw", "dng", "cr2", "raw", "orf", "rw2"]).blocking_pick_file() {
        Some(file) => Ok(Some(path_string(file)?)),
        None => Ok(None),
    }
}
...
fn build_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    let quit = MenuItem::with_id(app, "quit_saved", "保存并退出", true, None::<&str>)?;
    ...
    let show_changes = MenuItem::with_id(app, "show_changes", "版本与更新日志", true, None::<&str>)?;
    let open_github =
...
const GREEN_MANIFEST_URL: &str =
    "https://github.com/xiangyizhang-hue/watermark-house/releases/latest/download/green-latest.json";
...
// Android 原生导出桥：Rust 命令保持稳定，具体的 MediaStore 写入放在 Kotlin 插件，
// 这样 WebView 不需要持有 Android API，也能在后续版本把 base64 过渡为文件流/Channel。
#[cfg(target_os = "android")]
#[derive(serde::Serialize)]
struct MobileSaveMediaArgs {
    filename: String,
    mime: String,
...
#[cfg(target_os = "android")]
#[derive(serde::Serialize)]
struct MobileWriteMediaChunkArgs {
    token: String,
    #[serde(rename = "dataBase64")]
    data_base64: String,
}
...
#[cfg(target_os = "android")]
#[derive(serde::Serialize)]
struct MobileMediaTokenArgs {
    token: String,
}

#[cfg(target_os = "android")]
#[derive(serde::Serialize)]
struct MobileLocalRecordArgs {
    key: String,
    value: String,
    version: u32,
}
...
#[cfg(target_os = "android")]
#[derive(serde::Serialize)]
struct MobileLocalRecordKeyArgs {
    key: String,
}

#[cfg(target_os = "android")]
struct FramelabMobilePlugin<R: tauri::Runtime>(tauri::plugin::PluginHandle<R>);

#[cfg(mobile)]
fn framelab_mobile_plugin<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    tauri::plugin::Builder::new("framelab-mobile")
        .setup(|app, api| {
            #[cfg(target_os = "android")]
            {
                let handle = api.register_android_plugin("com.framelab.app", "FrameLabPlugin")?;
                app.manage(FramelabMobilePlugin(handle));
            }
            Ok(())
        })
        .build()
...
/// 将当前已经渲染完成的图片交给 Android MediaStore。
///
/// 目前前端只把不超过 32MB 的单张成品通过 IPC 传入，避免 WebView 产生多份大图；
/// 更大的原图导出仍会走浏览器下载兜底，后续原生分块通道接入后再取消该限制。
#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_save_media<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    filename: String,
    mime: String,
    data_base64: String,
) -> Result<String, String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        .0
        ...
        .await
        .map_err(|e| format!("Android 相册保存失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_begin_media<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    filename: String,
    mime: String,
) -> Result<String, String> {
    ...
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "beginMedia",
            MobileBeginMediaArgs { filename, mime },
        )
        .await
        .map_err(|e| format!("Android 流式保存初始化失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_write_media_chunk<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    token: String,
    data_base64: String,
) -> Result<(), String> {
    ...
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "writeMediaChunk",
            MobileWriteMediaChunkArgs { token, data_base64 },
        )
        .await
        .map(|_| ())
        .map_err(|e| format!("Android 流式保存分块失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_finish_media<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    token: String,
) -> Result<String, String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        .0
        .run_mobile_plugin_async::<String>("finishMedia", MobileMediaTokenArgs { token })
        .await
        .map_err(|e| format!("Android 流式保存完成失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_abort_media<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    token: String,
) -> Result<(), String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        .0
        .run_mobile_plugin_async::<String>("abortMedia", MobileMediaTokenArgs { token })
        .await
        .map(|_| ())
        .map_err(|e| format!("Android 流式保存中止失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_history<R: tauri::Runtime>(
    app: tauri::AppHandle<R>, operation: String, payload: String,
) -> Result<String, String> {
    app.state::<FramelabMobilePlugin<R>>().0
        .run_mobile_plugin_async::<String>("history", serde_json::json!({"operation": operation, "payload": payload}))
        .await.map_err(|e| format!("Android 历史存储失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_assets<R: tauri::Runtime>(
    app: tauri::AppHandle<R>, operation: String, payload: String,
) -> Result<String, String> {
    app.state::<FramelabMobilePlugin<R>>().0
        .run_mobile_plugin_async::<String>("assets", serde_json::json!({"operation": operation, "payload": payload}))
        .await.map_err(|e| format!("Android 图库存储失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_save_local_record<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    key: String,
    value: String,
    version: u32,
    ...
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "saveLocalRecord",
            MobileLocalRecordArgs { key, value, version },
        )
        .await
        .map(|_| ())
        .map_err(|e| format!("Android 本地记录保存失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_load_local_record<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    key: String,
    ...
    plugin
        ...
        .run_mobile_plugin_async::<Option<String>>(
            "loadLocalRecord",
            MobileLocalRecordKeyArgs { key },
        )
        .await
        .map_err(|e| format!("Android 本地记录读取失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_delete_local_record<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    key: String,
) -> Result<(), String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        ...
        .await
        .map(|_| ())
        .map_err(|e| format!("Android 本地记录删除失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_start_export_service<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    job_id: String,
    total: u32,
    completed: u32,
    ...
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "startExportService",
            MobileExportStartArgs { job_id, total, completed, label },
        )
        .await
        .map(|_| ())
        .map_err(|e| format!("启动 Android 后台导出失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_update_export_service<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    job_id: String,
    total: u32,
    completed: u32,
    ...
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "updateExportService",
            MobileExportUpdateArgs { job_id, total, completed, label },
        )
        .await
        .map(|_| ())
        .map_err(|e| format!("更新 Android 后台导出进度失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_cancel_export_service<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    job_id: String,
) -> Result<(), String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        ...
        .run_mobile_plugin_async::<String>(
            "cancelExportService",
            MobileExportJobArgs { job_id },
        )
        .await
        .map(|_| ())
        .map_err(|e| format!("取消 Android 后台导出失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_is_export_cancelled<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    job_id: String,
) -> Result<bool, String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        ...
        .await
        .map_err(|e| format!("读取 Android 导出取消状态失败: {e}"))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn mobile_stop_export_service<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    job_id: String,
) -> Result<(), String> {
    let plugin = app.state::<FramelabMobilePlugin<R>>();
    plugin
        ...
        .await
        .map(|_| ())
        .map_err(|e| format!("结束 Android 后台导出失败: {e}"))
}
...
pub fn run() {
    ...
    #[cfg(mobile)]
    {
        builder = builder.plugin(framelab_mobile_plugin());
    }

    // Android 不注册桌面单实例/菜单/对话框/updater 插件。移动端窗口由 Tauri
    // 的 mobile entry 自动创建，文件导入由前端 Photo Picker（input[type=file]）完成。
    #[cfg(desktop)]
    {
        ...
        builder = builder
            ...
            // 自动更新：仅桌面端经 @tauri-apps/plugin-updater 检查/下载/安装。
            ...
            .setup(|app| {
                ...
                // 环境变量（API 优先），此处合并回来，保证 scripts/cdp-shot.mjs 文档中的调试流程可用；
                // 正式构建（release）不生效。
                ...
                .title(format!("水印小屋 v{}", app.package_info().version))
    ...
    #[cfg(target_os = "android")]
    {
        builder = builder
            .invoke_handler(tauri::generate_handler![
                ...
                mobile_save_media,
                mobile_begin_media,
                mobile_write_media_chunk,
                mobile_finish_media,
                mobile_abort_media,
                mobile_save_local_record,
                mobile_history,
                mobile_assets,
                mobile_load_local_record,
                mobile_delete_local_record,
                mobile_start_export_service,
                mobile_update_export_service,
                mobile_cancel_export_service,
                mobile_is_export_cancelled,
                mobile_stop_export_service,
            ]);
    }
    ...
    // Tauri 的 mobile entry point 只负责把 Rust 运行时接到 Android/iOS
    // Activity，并不会自动创建 WebView。此前移动端分支直接返回 Ok(())，
    // 导致 Activity 只有原生空白背景，前端 index.html 根本没有加载。
    // 移动端必须显式创建唯一的 main WebView。
    #[cfg(mobile)]
    {
